...
# Here we will also run an MCMC algorithm to fit the data but adding a prior to restrain tau
# We use the result from the Planck satellite as prior on Tau -> 0.0544 +/- 0.0073

# So we basically use the exact same code, but adding an acceptance condition on Tau
...
f = open('chains_chi2_p4_nstep2500_scale05.txt', 'w')
# we define our data
data = wmap[:,1]
# we define the noise
noise = wmap[:,2]
...
# We also put the planck value for tau
pars=np.asarray([71,0.02,0.1,0.0544,2e-9,0.96])
params = pars
...
for i in range(nstep):
    ...
    # we discard points outside of 3 sigma of the accepted value as our prior
    if new_params[3] > 0.0544-3*0.0073 and new_params[3] < 0.0544+3*0.0073:
